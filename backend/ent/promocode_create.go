...
func (_c *PromoCodeCreate) check() error {
	if _, ok := _c.mutation.Code(); !ok {
		return &ValidationError{Name: "code", err: errors.New(`ent_tmp: missing required field "PromoCode.code"`)}
	}
	if v, ok := _c.mutation.Code(); ok {
		if err := promocode.CodeValidator(v); err != nil {
			return &ValidationError{Name: "code", err: fmt.Errorf(`ent_tmp: validator failed for field "PromoCode.code": %w`, err)}
		}
	}
	if _, ok := _c.mutation.BonusAmount(); !ok {
		return &ValidationError{Name: "bonus_amount", err: errors.New(`ent_tmp: missing required field "PromoCode.bonus_amount"`)}
	}
	if _, ok := _c.mutation.MaxUses(); !ok {
		return &ValidationError{Name: "max_uses", err: errors.New(`ent_tmp: missing required field "PromoCode.max_uses"`)}
	}
	if _, ok := _c.mutation.UsedCount(); !ok {
		return &ValidationError{Name: "used_count", err: errors.New(`ent_tmp: missing required field "PromoCode.used_count"`)}
	}
	if _, ok := _c.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`ent_tmp: missing required field "PromoCode.status"`)}
	}
	if v, ok := _c.mutation.Status(); ok {
		if err := promocode.StatusValidator(v); err != nil {
			return &ValidationError{Name: "status", err: fmt.Errorf(`ent_tmp: validator failed for field "PromoCode.status": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent_tmp: missing required field "PromoCode.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent_tmp: missing required field "PromoCode.updated_at"`)}
	}
	return nil
}
...
func (u *PromoCodeUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for PromoCodeCreate.OnConflict")
	}
...
func (u *PromoCodeUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent_tmp: OnConflict was set for builder %d. Set it on the PromoCodeCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for PromoCodeCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
