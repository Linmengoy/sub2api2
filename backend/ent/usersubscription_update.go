...
func (_u *UserSubscriptionUpdate) defaults() error {
	if _, ok := _u.mutation.UpdatedAt(); !ok {
		if usersubscription.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent_tmp: uninitialized usersubscription.UpdateDefaultUpdatedAt (forgotten import ent_tmp/runtime?)")
		}
...
func (_u *UserSubscriptionUpdate) check() error {
	...
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UserSubscription.user"`)
	}
	if _u.mutation.GroupCleared() && len(_u.mutation.GroupIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UserSubscription.group"`)
	}
	return nil
}
...
func (_u *UserSubscriptionUpdateOne) sqlSave(ctx context.Context) (_node *UserSubscription, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent_tmp: missing "UserSubscription.id" for update`)}
	}
