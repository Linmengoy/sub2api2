...
func (_u *APIKeyUpdate) defaults() error {
	if _, ok := _u.mutation.UpdatedAt(); !ok {
		if apikey.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent_tmp: uninitialized apikey.UpdateDefaultUpdatedAt (forgotten import ent_tmp/runtime?)")
		}
...
func (_u *APIKeyUpdate) check() error {
	...
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "APIKey.user"`)
	}
	return nil
}
...
func (_u *APIKeyUpdateOne) sqlSave(ctx context.Context) (_node *APIKey, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent_tmp: missing "APIKey.id" for update`)}
	}
