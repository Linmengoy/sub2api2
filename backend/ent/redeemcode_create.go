...
func (_c *RedeemCodeCreate) check() error {
	if _, ok := _c.mutation.Code(); !ok {
		return &ValidationError{Name: "code", err: errors.New(`ent_tmp: missing required field "RedeemCode.code"`)}
	}
	if v, ok := _c.mutation.Code(); ok {
		if err := redeemcode.CodeValidator(v); err != nil {
			return &ValidationError{Name: "code", err: fmt.Errorf(`ent_tmp: validator failed for field "RedeemCode.code": %w`, err)}
		}
	}
	if _, ok := _c.mutation.GetType(); !ok {
		return &ValidationError{Name: "type", err: errors.New(`ent_tmp: missing required field "RedeemCode.type"`)}
	}
	if v, ok := _c.mutation.GetType(); ok {
		if err := redeemcode.TypeValidator(v); err != nil {
			return &ValidationError{Name: "type", err: fmt.Errorf(`ent_tmp: validator failed for field "RedeemCode.type": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Value(); !ok {
		return &ValidationError{Name: "value", err: errors.New(`ent_tmp: missing required field "RedeemCode.value"`)}
	}
	if _, ok := _c.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`ent_tmp: missing required field "RedeemCode.status"`)}
	}
	if v, ok := _c.mutation.Status(); ok {
		if err := redeemcode.StatusValidator(v); err != nil {
			return &ValidationError{Name: "status", err: fmt.Errorf(`ent_tmp: validator failed for field "RedeemCode.status": %w`, err)}
		}
	}
	if _, ok := _c.mutation.PurchaseAmount(); !ok {
		return &ValidationError{Name: "purchase_amount", err: errors.New(`ent_tmp: missing required field "RedeemCode.purchase_amount"`)}
	}
	if _, ok := _c.mutation.PurchasePayAmount(); !ok {
		return &ValidationError{Name: "purchase_pay_amount", err: errors.New(`ent_tmp: missing required field "RedeemCode.purchase_pay_amount"`)}
	}
	if v, ok := _c.mutation.PurchaseCurrency(); ok {
		if err := redeemcode.PurchaseCurrencyValidator(v); err != nil {
			return &ValidationError{Name: "purchase_currency", err: fmt.Errorf(`ent_tmp: validator failed for field "RedeemCode.purchase_currency": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent_tmp: missing required field "RedeemCode.created_at"`)}
	}
	if _, ok := _c.mutation.ValidityDays(); !ok {
		return &ValidationError{Name: "validity_days", err: errors.New(`ent_tmp: missing required field "RedeemCode.validity_days"`)}
	}
	return nil
}
...
func (u *RedeemCodeUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for RedeemCodeCreate.OnConflict")
	}
...
func (u *RedeemCodeUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent_tmp: OnConflict was set for builder %d. Set it on the RedeemCodeCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for RedeemCodeCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
