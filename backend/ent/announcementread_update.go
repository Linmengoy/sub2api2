...
func (_u *AnnouncementReadUpdate) check() error {
	if _u.mutation.AnnouncementCleared() && len(_u.mutation.AnnouncementIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "AnnouncementRead.announcement"`)
	}
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "AnnouncementRead.user"`)
	}
	return nil
}
...
func (_u *AnnouncementReadUpdateOne) sqlSave(ctx context.Context) (_node *AnnouncementRead, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent_tmp: missing "AnnouncementRead.id" for update`)}
	}
