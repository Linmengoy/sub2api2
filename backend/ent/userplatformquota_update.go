...
func (_u *UserPlatformQuotaUpdate) defaults() error {
	if _, ok := _u.mutation.UpdatedAt(); !ok {
		if userplatformquota.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent_tmp: uninitialized userplatformquota.UpdateDefaultUpdatedAt (forgotten import ent_tmp/runtime?)")
		}
...
func (_u *UserPlatformQuotaUpdate) check() error {
	...
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UserPlatformQuota.user"`)
	}
	return nil
}
...
func (_u *UserPlatformQuotaUpdateOne) sqlSave(ctx context.Context) (_node *UserPlatformQuota, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent_tmp: missing "UserPlatformQuota.id" for update`)}
	}
