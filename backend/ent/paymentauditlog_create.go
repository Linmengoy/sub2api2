...
func (_c *PaymentAuditLogCreate) check() error {
	if _, ok := _c.mutation.OrderID(); !ok {
		return &ValidationError{Name: "order_id", err: errors.New(`ent_tmp: missing required field "PaymentAuditLog.order_id"`)}
	}
	if v, ok := _c.mutation.OrderID(); ok {
		if err := paymentauditlog.OrderIDValidator(v); err != nil {
			return &ValidationError{Name: "order_id", err: fmt.Errorf(`ent_tmp: validator failed for field "PaymentAuditLog.order_id": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Action(); !ok {
		return &ValidationError{Name: "action", err: errors.New(`ent_tmp: missing required field "PaymentAuditLog.action"`)}
	}
	if v, ok := _c.mutation.Action(); ok {
		if err := paymentauditlog.ActionValidator(v); err != nil {
			return &ValidationError{Name: "action", err: fmt.Errorf(`ent_tmp: validator failed for field "PaymentAuditLog.action": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Detail(); !ok {
		return &ValidationError{Name: "detail", err: errors.New(`ent_tmp: missing required field "PaymentAuditLog.detail"`)}
	}
	if _, ok := _c.mutation.Operator(); !ok {
		return &ValidationError{Name: "operator", err: errors.New(`ent_tmp: missing required field "PaymentAuditLog.operator"`)}
	}
	if v, ok := _c.mutation.Operator(); ok {
		if err := paymentauditlog.OperatorValidator(v); err != nil {
			return &ValidationError{Name: "operator", err: fmt.Errorf(`ent_tmp: validator failed for field "PaymentAuditLog.operator": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent_tmp: missing required field "PaymentAuditLog.created_at"`)}
	}
	return nil
}
...
func (u *PaymentAuditLogUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for PaymentAuditLogCreate.OnConflict")
	}
...
func (u *PaymentAuditLogUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent_tmp: OnConflict was set for builder %d. Set it on the PaymentAuditLogCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for PaymentAuditLogCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
