...
func (_u *UsageLogUpdate) check() error {
	...
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UsageLog.user"`)
	}
	if _u.mutation.APIKeyCleared() && len(_u.mutation.APIKeyIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UsageLog.api_key"`)
	}
	if _u.mutation.AccountCleared() && len(_u.mutation.AccountIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UsageLog.account"`)
	}
	return nil
}
...
func (_u *UsageLogUpdateOne) sqlSave(ctx context.Context) (_node *UsageLog, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent_tmp: missing "UsageLog.id" for update`)}
	}
	...
	if fields := _u.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !usagelog.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("ent_tmp: invalid field %q for query", f)}
			}
