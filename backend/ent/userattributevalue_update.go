...
func (_u *UserAttributeValueUpdate) check() error {
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UserAttributeValue.user"`)
	}
	if _u.mutation.DefinitionCleared() && len(_u.mutation.DefinitionIDs()) > 0 {
		return errors.New(`ent_tmp: clearing a required unique edge "UserAttributeValue.definition"`)
	}
	return nil
}
...
func (_u *UserAttributeValueUpdateOne) sqlSave(ctx context.Context) (_node *UserAttributeValue, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent_tmp: missing "UserAttributeValue.id" for update`)}
	}
