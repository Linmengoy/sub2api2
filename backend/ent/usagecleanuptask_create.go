...
func (_c *UsageCleanupTaskCreate) check() error {
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent_tmp: missing required field "UsageCleanupTask.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent_tmp: missing required field "UsageCleanupTask.updated_at"`)}
	}
	if _, ok := _c.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`ent_tmp: missing required field "UsageCleanupTask.status"`)}
	}
	if v, ok := _c.mutation.Status(); ok {
		if err := usagecleanuptask.StatusValidator(v); err != nil {
			return &ValidationError{Name: "status", err: fmt.Errorf(`ent_tmp: validator failed for field "UsageCleanupTask.status": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Filters(); !ok {
		return &ValidationError{Name: "filters", err: errors.New(`ent_tmp: missing required field "UsageCleanupTask.filters"`)}
	}
	if _, ok := _c.mutation.CreatedBy(); !ok {
		return &ValidationError{Name: "created_by", err: errors.New(`ent_tmp: missing required field "UsageCleanupTask.created_by"`)}
	}
	if _, ok := _c.mutation.DeletedRows(); !ok {
		return &ValidationError{Name: "deleted_rows", err: errors.New(`ent_tmp: missing required field "UsageCleanupTask.deleted_rows"`)}
	}
	return nil
}
...
func (u *UsageCleanupTaskUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for UsageCleanupTaskCreate.OnConflict")
	}
...
func (u *UsageCleanupTaskUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent_tmp: OnConflict was set for builder %d. Set it on the UsageCleanupTaskCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for UsageCleanupTaskCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
