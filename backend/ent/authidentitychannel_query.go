...
func (_q *AuthIdentityChannelQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("ent_tmp: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (_q *AuthIdentityChannelQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range _q.inters {
		if inter == nil {
			return fmt.Errorf("ent_tmp: uninitialized interceptor (forgotten import ent_tmp/runtime?)")
		}
	...
	for _, f := range _q.ctx.Fields {
		if !authidentitychannel.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("ent_tmp: invalid field %q for query", f)}
		}
	}
	if _q.path != nil {
