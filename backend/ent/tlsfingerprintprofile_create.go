...
func (_c *TLSFingerprintProfileCreate) check() error {
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent_tmp: missing required field "TLSFingerprintProfile.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent_tmp: missing required field "TLSFingerprintProfile.updated_at"`)}
	}
	if _, ok := _c.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`ent_tmp: missing required field "TLSFingerprintProfile.name"`)}
	}
	if v, ok := _c.mutation.Name(); ok {
		if err := tlsfingerprintprofile.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`ent_tmp: validator failed for field "TLSFingerprintProfile.name": %w`, err)}
		}
	}
	if _, ok := _c.mutation.EnableGrease(); !ok {
		return &ValidationError{Name: "enable_grease", err: errors.New(`ent_tmp: missing required field "TLSFingerprintProfile.enable_grease"`)}
	}
	return nil
}
...
func (u *TLSFingerprintProfileUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for TLSFingerprintProfileCreate.OnConflict")
	}
...
func (u *TLSFingerprintProfileUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent_tmp: OnConflict was set for builder %d. Set it on the TLSFingerprintProfileCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for TLSFingerprintProfileCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
