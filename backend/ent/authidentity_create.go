...
func (_c *AuthIdentityCreate) check() error {
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent_tmp: missing required field "AuthIdentity.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent_tmp: missing required field "AuthIdentity.updated_at"`)}
	}
	if _, ok := _c.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`ent_tmp: missing required field "AuthIdentity.user_id"`)}
	}
	if _, ok := _c.mutation.ProviderType(); !ok {
		return &ValidationError{Name: "provider_type", err: errors.New(`ent_tmp: missing required field "AuthIdentity.provider_type"`)}
	}
	if v, ok := _c.mutation.ProviderType(); ok {
		if err := authidentity.ProviderTypeValidator(v); err != nil {
			return &ValidationError{Name: "provider_type", err: fmt.Errorf(`ent_tmp: validator failed for field "AuthIdentity.provider_type": %w`, err)}
		}
	}
	if _, ok := _c.mutation.ProviderKey(); !ok {
		return &ValidationError{Name: "provider_key", err: errors.New(`ent_tmp: missing required field "AuthIdentity.provider_key"`)}
	}
	if v, ok := _c.mutation.ProviderKey(); ok {
		if err := authidentity.ProviderKeyValidator(v); err != nil {
			return &ValidationError{Name: "provider_key", err: fmt.Errorf(`ent_tmp: validator failed for field "AuthIdentity.provider_key": %w`, err)}
		}
	}
	if _, ok := _c.mutation.ProviderSubject(); !ok {
		return &ValidationError{Name: "provider_subject", err: errors.New(`ent_tmp: missing required field "AuthIdentity.provider_subject"`)}
	}
	if v, ok := _c.mutation.ProviderSubject(); ok {
		if err := authidentity.ProviderSubjectValidator(v); err != nil {
			return &ValidationError{Name: "provider_subject", err: fmt.Errorf(`ent_tmp: validator failed for field "AuthIdentity.provider_subject": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Metadata(); !ok {
		return &ValidationError{Name: "metadata", err: errors.New(`ent_tmp: missing required field "AuthIdentity.metadata"`)}
	}
	if len(_c.mutation.UserIDs()) == 0 {
		return &ValidationError{Name: "user", err: errors.New(`ent_tmp: missing required edge "AuthIdentity.user"`)}
	}
	return nil
}
...
func (u *AuthIdentityUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for AuthIdentityCreate.OnConflict")
	}
...
func (u *AuthIdentityUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent_tmp: OnConflict was set for builder %d. Set it on the AuthIdentityCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent_tmp: missing options for AuthIdentityCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
